...
- **Agent Skills**（`npx skills add ...`）：让 Codex 安装 `share-codex-chat` 或 `share-claude-code-chat` 后，把 Codex / Claude Code 会话导出成高保真 HTML 并部署分享
...
| Agent Skills | Agent Skills / Codex（`skills/share-codex-chat`、`skills/share-claude-code-chat`） |
| 基础设施 | 阿里云 ACK（K8s） |
...
├── skills/                  # Agent Skills（如 share-codex-chat、share-claude-code-chat）
└── scripts/                 # 本地开发辅助脚本
```

## Agent Skills

PreviewShip 可以作为 AI 编程对话的原生分享能力使用，而不只是一个部署命令。仓库内置的 `share-codex-chat` 和 `share-claude-code-chat` skills 会把 Codex 或 Claude Code 会话导出为高保真 HTML 页面，并通过 PreviewShip CLI 部署成公开 URL，适合分享调试过程、实现记录、工具调用脉络、评审上下文和 AI 协作成果。
...
该 skill 会读取 `~/.claude/projects` 下的 Claude Code JSONL 会话，渲染可见消息和安全的工具活动时间线，隐藏思考原文与原始工具输出，并通过 PreviewShip CLI 部署生成的 HTML。
