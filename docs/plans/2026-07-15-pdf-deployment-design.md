...
首版使用浏览器原生 PDF 阅读能力，不引入 PDF.js。预览页不增加 PreviewShip 顶部工具栏；PDF 占满整个视口，Free 套餐继续显示现有悬浮品牌标识，Pro 套餐不显示。
...
- 后端规范化产物后保留 `document.pdf`，并生成根目录 `index.html` 作为全屏阅读入口。
- `index.html` 使用浏览器原生 PDF 查看器加载 `document.pdf`，同时为不支持内嵌 PDF 的浏览器提供打开原文件的降级入口。
...
4. 后端校验 PDF 文件签名以排除伪装扩展名，并生成全屏 `index.html`。
5. 现有品牌注入逻辑只修改生成的 HTML，不修改 PDF 二进制内容。
...
- 浏览器不支持内嵌 PDF：全屏入口显示简洁的“打开 PDF”链接，仍可访问原文件。
...
- 固定预览链接以原生查看器全屏打开 PDF，没有 PreviewShip 顶部工具栏。
- Free 显示现有悬浮品牌标识，Pro 不显示。
